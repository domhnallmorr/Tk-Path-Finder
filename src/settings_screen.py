...
class SettingsWindow(ttk.Frame):
	...
	def setup_open_with(self):
		ttk.Label(self.top, text='File Extension:').grid(row=0, column=0, columnspan=1, sticky='NSEW', pady=self.mainapp.default_pady)
		self.extension_entry = ttk.Entry(self.top)
		...
		ttk.Label(self.top, text='App Path:').grid(row=1, column=0, columnspan=1, sticky='NSEW', pady=self.mainapp.default_pady)
		self.app_entry = ttk.Entry(self.top)
		...
		# submit button
		self.submit = ttk.Button(self.top, text='Add', style='success.TButton', command=self.add)
		self.submit.grid(row=3, column=0, sticky='ew', pady=10, padx=(0, 10))
		
		self.edit = ttk.Button(self.top, text='Edit Selected', style='info.TButton', command=self.edit_row)
		self.edit.grid(row=3, column=1, sticky='ew', pady=10, padx=(0, 10))

		self.delete_btn = ttk.Button(self.top, text='Delete Selected', style='danger.TButton', command=self.delete_row)
		...
		self.treeview = treeview_functions.create_treeview(self.top, column_names, column_widths, height)
		...
		vsb = autoscrollbar.AutoScrollbar(self.top, orient="vertical", command=self.treeview.yview)
