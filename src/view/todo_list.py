...
class ToDoListWindow(ttk.Frame):
	def __init__(self, master, view, to_do_list):
	...
	def setup_label_frames(self):
		...
		self.todo_frame.grid(row=2, column=0, columnspan=8, rowspan=2, sticky="NSEW", padx=self.view.default_padx, pady=5, ipadx=2, ipady=5)
	...
	def setup_treeview(self):
		...
		treeview_functions.write_data_to_treeview_general(self.treeview, 'replace', self.to_do_list)
		self.update_tags()
	...
	def setup_input_widgets(self):
		...
		self.status_combo.grid(row=1, column=1, columnspan=1, sticky='NSEW', padx=self.view.default_padx, pady=self.view.default_pady)
		
	def setup_buttons(self):
		# submit button
		self.submit = ttk.Button(self.todo_frame, text='Add', width=15, style='success.TButton', command=self.add)
		self.submit.grid(row=3, column=0, sticky='ew', padx=self.view.default_padx, pady=self.view.default_pady)
		
		self.edit = ttk.Button(self.todo_frame, text='Edit Selected', width=15, style='info.TButton', command=self.edit_row)
		self.edit.grid(row=3, column=1, sticky='ew', padx=self.view.default_padx, pady=self.view.default_pady)

		self.delete_btn = ttk.Button(self.todo_frame, text='Delete Selected', width=15, style='danger.TButton', command=self.delete_row)
		self.delete_btn.grid(row=3, column=2, sticky='ew', padx=self.view.default_padx, pady=self.view.default_pady)
		
		self.delete_all_btn = ttk.Button(self.todo_frame, text='Delete All', width=15, style='danger.TButton', command=self.delete_all)
		self.delete_all_btn.grid(row=3, column=15, sticky='ew', padx=self.view.default_padx, pady=self.view.default_pady)
		
	def add(self):
		task = self.task_entry.get()
		self.to_do_list.append([len(self.to_do_list)+1, "Open", task])
	...
	def edit_row(self):
		...
		self.to_do_list[current_idx] = [self.treeview.item(selected_iid, 'text'), self.status_combo.get(), self.task_entry.get()]
		self.update_tags()
	...
	def delete_row(self):
		...
		if msg:
			...
			self.to_do_list.pop(current_idx)
			
			# renumber all items
			for idx, task in enumerate(self.to_do_list):
				self.to_do_list[idx][0] = idx + 1
